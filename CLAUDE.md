...
Two separate binaries sharing the `shared/update` package. Uses **Cobra** for CLI and **Chi** for HTTP routing.
...
- **internal/service/stats.go**: Periodic (3s) formatted log display of connected client stats (speed, buffer, position).

### Client (`syncclient/`)
- **main.go**: Cobra CLI. Connects to host WS, receives `init` message with available variants, shows interactive selection menu, launches MPV, bridges MPV IPC <-> WebSocket for bidirectional sync. Sends periodic stats reports (2s). Uses atomic `applyingCount` to prevent echo loops.
